...
Lets load packages we will be using today. knitr is available on CRAN and you can install robustToxicities from github directly: `devtools::install_github("finite2/robustToxicities")`. Note than you may need to install the devtools package first. We then load the example data and have a look.

```{r, warning = FALSE}
library(robustToxicities)
library(knitr)
data = read.csv("Fake_data_time.csv", stringsAsFactors = FALSE)
str(data, vec.len = 2)
```
...
toxPlot(toxDB, rowID_range = c(1,8))

toxPlot(toxDB, rowID_range = c(9,16))
```

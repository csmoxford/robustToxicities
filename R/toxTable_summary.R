...
toxTable_summary=function(toxDB){
  ...
  if(a==0){
    ...
    for(treatment in 1:length(treats)){
      ...
      tox.db.sub=toxDB@cleanData[toxDB@cleanData$treatment==treats[treatment],]
      ...
      for(cycle.no in 1:length(cycle.lists)){
        ...
        if(length(cycles)>1){
          toxTable[cycle.no,7*treatment-5]=length(unique(tox.db.sub$patid[apply(tox.db.sub[,paste0("cycle_start_date",cycles)],1,function(x) sum(!is.na(x))>0)]))
          x=aggregate(apply(tox.db.sub[,paste0("occur_in_cycle_",cycles)],1,max),by=list(tox.db.sub$patid),FUN=max)$x
        } else {
          toxTable[cycle.no,7*treatment-5]=length(unique(tox.db.sub$patid[is.na(tox.db.sub[,paste0("cycle_start_date",cycles)])==FALSE]))
          x=aggregate(tox.db.sub[,paste0("occur_in_cycle_",cycles)],by=list(tox.db.sub$patid),FUN=max)$x
        }
        toxTable[cycle.no,4:8+(treatment-1)*7]=c(sum(x==1),sum(x==2),sum(x==3),sum(x==4),sum(x==5))
        toxTable[cycle.no,3+(treatment-1)*7]=toxTable[cycle.no,2+(treatment-1)*7]-sum(toxTable[cycle.no,4:8+(treatment-1)*7])
      }
    }

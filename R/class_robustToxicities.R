...
defaultToxicityOptions = function(trialName, folderPath = NULL, outputFolder = NULL) {
  .toxicityOptions(
    ...
    cycleCategoryMerge=character(0), # Not implimented I don't think
    outputFolder=outputFolder
  )
}

#' @exportClass robustToxicities
.robustToxicities = setClass("robustToxicities",slots=c(data="data.frame", cleanData="data.frame", treatmentLabels="data.frame", timeType="character", cycleLabels="data.frame", queries="data.frame", options = "toxicityOptions"))

#' @export robustToxicities
robustToxicities = function(data, cycleLabels, options, treatmentLabels, timeType = "time") {
  ...
  # treaments
  if(dim(treatmentLabels)[1] > 1 & !"treatment" %in% name){
    stop("treatment column not found in database")
  } else if(dim(treatmentLabels)[1] ==1 & !"treatment" %in% name){
...
labels=function(label, index) {
  return(data.frame(label = label, index=index))
}

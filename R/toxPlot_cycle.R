...
toxPlot_cycle = function(toxDB, patients = character(0), plot=TRUE) {
  ...
  if (toxDB@options@timeType != "cycle") {
    stop("This function only works for cycle type data")
  }
  ...
  names_present = names(toxDB@cleanData)[str_detect(names(toxDB@cleanData), "occur_in_cycle_")]

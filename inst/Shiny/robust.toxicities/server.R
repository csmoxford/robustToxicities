    ...
    if (!is.null(input$table.update)) {
      if (input$table.update>0) {
        isolate({
          ...
          if (dim(values$toxDB@cleanData)[1]>0) {
            tox.table = toxTable_worstInCycle(values$toxDB,cycles=cycles,worst = input$worst)
        ...
        isolate({
          if (!is.null(values$toxDB)) {
            ...
            if (file.exists(input$listing.path)) {
              tox.full.db = values$toxDB[values$toxDB@cleanData$ass_TRUE == TRUE,]
              if (dim(tox.full.db)[1]>0) {
                ...
                for(i in 1:length(values$plot.cycle.merge)) {
                  ...
                  addParagraph(rtffile, paste0("Toxicities Cycle:",cycles,""))
    ...
    if (!is.null(input$sum.table.update)) {
      if (input$sum.table.update>0) {
        isolate({
          ...
          tox.full.db = values$toxDB[values$toxDB@cleanData$ass_TRUE == TRUE,]
          if (dim(tox.full.db)[1]>0) {
            output$summary = renderTable({toxTable_worstSummary(tox.full.db,cycleMerge = input$sum.cycle.merge,columnMerge = input$sum.col.merge)},digits  =  0,include.rownames = FALSE)
          } else {
            message("No matched data in database, table not created")
            message("################################################################")
          }
    ...
    if (!is.null(input$sum.table.save)) {
      if (input$sum.table.save>0) {
        isolate({
          if (!is.null(values$toxDB)) {
            tox.full.db = values$toxDB[values$toxDB@cleanData$ass_TRUE == TRUE,]
            sum.table = toxTable_worstSummary(tox.full.db,cycleMerge = input$sum.cycle.merge,columnMerge = input$sum.col.merge)
            if (file.exists(input$sum.path)) {
              rtffile <- RTF(paste0(nput$sum.path,"/",input$trial.name,"_toxity_summary.doc"))  # this can be an .rtf or a .doc
        ...
        isolate({
          if (is.null(values$toxDB)) {
            ...
            if (input$plot.height == 0) {
              plot.height = plot_toxicityTime(values$toxDB,dayRange = c(input$plot.minday,input$plot.maxday),plot = FALSE)*18+180
            ...
            output$Toxicity = renderPlot({isolate({plot_toxicityTime(values$toxDB,dayRange = c(input$plot.minday,input$plot.maxday),omit = 0,plotCycleLength = input$plot.cycle.length)})},height = plot.height,width = plot.width)
          }
          message("################################################################")

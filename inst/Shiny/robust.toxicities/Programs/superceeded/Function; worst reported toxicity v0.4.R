...
worst.toxicity=function(values,input){

  # databases
  tox.full.db=values$trial.tox.db
  tox.db=values$trial.tox.db
  ############################################################################################
  # subset only matched data
  tox.db=tox.db[tox.db$ass_TRUE==TRUE,]
  tox.full.db=tox.full.db[tox.full.db$ass_TRUE==TRUE,]
  ...
  if(a==0){
    ...
    for(treatment in 1:length(treats)){
      ...
      tox.db=tox.full.db[tox.full.db$treatment==treats[treatment],]
      ...
      for(cycle.no in 1:length(cycle.lists)){
        ...
        if(length(cycles)>1){
          tox.table[cycle.no,7*treatment-5]=length(unique(tox.full.db$patid[apply(tox.full.db[,paste0("cycle_start_date",cycles)],1,function(x) sum(!is.na(x))>0)]))
          x=aggregate(apply(tox.full.db[,paste0("occur_in_cycle_",cycles)],1,max),by=list(tox.full.db$patid),FUN=max)$x
        } else {
          tox.table[cycle.no,7*treatment-5]=length(unique(tox.full.db$patid[is.na(tox.full.db[,paste0("cycle_start_date",cycles)])==FALSE]))
          x=aggregate(tox.full.db[,paste0("occur_in_cycle_",cycles)],by=list(tox.full.db$patid),FUN=max)$x
          print(x)
        }
